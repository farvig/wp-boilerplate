  ...
  public function __construct ( $meta_box ) {
    ...
    if (isset($meta_box['use_with_theme']))
      if ($meta_box['use_with_theme'] === true){
        $this->SelfPath = get_stylesheet_directory_uri() . '/inc/lib/meta-box-class';
  ...
  public function show_field_repeater( $field, $meta ) {
    ...
    $meta = get_post_meta($post->ID,$field['id'],true);
      ...
      if (count($meta) > 0 && is_array($meta) ){
        ...
        echo '</table>';
        if ($field['sortable'])
          echo '<span class="re-control button"><span class="at_re_sort_handle">|||</span></span>';

        echo'
        <span class="re-control at-re-toggle button"><img src="'.$plugin_path.'/images/edit.png" alt="Edit" title="Edit"/></span> 
        <span class="re-control button"><img src="'.$plugin_path.'/images/remove.png" alt="'.__('Remove','mmb').'" title="'.__('Remove','mmb').'" id="remove-'.$field['id'].'"></span>
        <span class="re-control-clear"></span></div>';
        $c = $c + 1;
        }
      }
    echo '<span class="button" id="add-'.$field['id'].'">'.__('Add','mmb').'</span>';
    echo '<br/></div>';
...
.at-repater-block{background-color: #FFFFFF;border: 1px solid;margin: 2px; min-height: 50px}
  ...
  public function show_field_text( $field, $meta) {  
    $this->show_field_begin( $field, $meta );
    echo "<input type='text' class='at-text widefat".( isset($field['class'])? ' ' . $field['class'] : '' )."' name='{$field['id']}' id='{$field['id']}' value='{$meta}' size='30' ".( isset($field['style'])? "style='{$field['style']}'" : '' )."/>";
    $this->show_field_end( $field, $meta );
  }
